...
namespace Event {
    ...
    void EventNoBufferChannel::update() {
    ...
    bool EventNoBufferChannel::eventSet(const std::shared_ptr<EventLoop> &loop) {
        int ret = 0;
        if (!events) {
            LOG_CONTENT_ERR("events is nullptr")
            return false;
        }

        if ((events & EV_READ) && (events & EV_WRITE)) {
            LOG_CONTENT_ERR("events is error!")
            return false;
        }
        ptr = event_new(loop->getEventBase(), getChannelFd(), events, onEvent, this);
        event_del(ptr.get());
        ...
        if (ret != 0) {
            LOG_CONTENT_ERR("event_add failed,fd:" + std::to_string(getChannelFd()))
            return false;
        }
        return true;;
    }
