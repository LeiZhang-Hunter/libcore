...
typedef std::function<void()> InitCallable;
...
public:
    ...
    *
    */
    virtual bool setDaemonize() {
        return false;
    }


    virtual void addInitCallable(const InitCallable &callable) {
     ...
     * @return pid_t
     */
    virtual pid_t getTid() {
        return tid;
    }

    //获取线程内部的事件循环
    virtual const std::shared_ptr<Event::EventLoop> &getEventLoop() {
        return loop;
    }
